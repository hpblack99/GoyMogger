...
class FFEQuoter:
    ...
    def _safe_goto(self, url: str, wait_for: str | None = None, attempts: int = 3) -> None:
        ...
        already interactive. We wait for 'domcontentloaded' (HTML parsed) and,
    ...
    def _is_browser_alive(self) -> bool:
        """Return True if the browser process and page are still usable."""
        try:
            _ = self.page.url
            return True
        except Exception:
    ...
    def _relogin_if_needed(self) -> bool:
        """If the current page is the login screen, silently re-authenticate.
    ...
    def _recover_session(self) -> None:
        """Recover from a crashed/closed browser or an expired session."""
